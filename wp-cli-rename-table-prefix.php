...
/*
Plugin Name: WP-CLI Rename Table Prefix
Plugin URI:  https://github.com/spxis/wp-cli-rename-table-prefix
Description: A WP-CLI command to rename WordPress' table prefix value in wp-config.php.
Version:     0.1
Author:      John Morris 
License:     GPLv2
*/
...
class WP_CLI_Rename_Table_Prefix extends \WP_CLI_Command {
	var $old_prefix, $new_prefix;

	/**
	 * Rename WordPress' table prefix.
	 *
	...
	public function __invoke( $args, $assoc_args ) {
		...
		try {
			...
			\WP_CLI::success( 'Successfully renamed table prefix.' );
	...
	protected function confirm() {
		...
		\WP_CLI::confirm( sprintf(
			"\nAre you sure you want to rename %s's table prefix from `%s` to `%s`?",
...
\WP_CLI::add_command( 'rename-table-prefix', 'WP_CLI_Rename_Table_Prefix' );
